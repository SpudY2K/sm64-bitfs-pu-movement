...
int find_wall(Vec3f position, Surface** wall, Surface wall_set[], int n_wall_set) {
	float x = position[0];
	float y = position[1] + (mario_hitbox_height / 2);
	float z = position[2];
	...
	for (int i = 0; i < n_wall_set; ++i) {
		if (y < wall_set[i].lower_y || y > wall_set[i].upper_y || x < wall_set[i].min_x - mario_radius || x > wall_set[i].max_x + mario_radius || z < wall_set[i].min_z - mario_radius || z > wall_set[i].max_z + mario_radius) {
			continue;
		}

		float offset = wall_set[i].normal[0] * x + wall_set[i].normal[1] * y + wall_set[i].normal[2] * z + wall_set[i].origin_offset;

		if (offset < -mario_radius || offset > mario_radius) {
			continue;
		}
		...
		position[0] += wall_set[i].normal[0] * (mario_radius - offset);
		position[2] += wall_set[i].normal[2] * (mario_radius - offset);
